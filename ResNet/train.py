...
if __name__ == '__main__':
    ...
    resnet = ResNet(input_var, training_var)
    print('# model build')

    learning_rate_var = tf.placeholder(tf.float32, name = 'learning_rate')
    
    #loss
    loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels = label_var, logits = resnet))
    ...
    #accuracy
    correct_prediction = tf.equal(tf.argmax(resnet, 1), tf.argmax(label_var, 1))
